...
@section('title',__('messages.id_2').' - '.__('messages.update_record'))
@section('page_title',__('messages.id_2').' - '.__('messages.update_record'))

@section('body')
    <div class="section-wrapper">
        @php
            $action = route('id2.update',$secondSheet->id);
        @endphp
        @include('panel.include.errors')
        <form action="{{route('id2.pricesUpdate',$secondSheet->id)}}" id="product_form" method="post">
            @csrf
            <input type="hidden" name="method_type" id="method_type" value="update">
            <input type="hidden" name="second_sheet_id" id="second_sheet_id" value="{{$secondSheet->id}}">
